...
class RelayServer extends EventEmitter {
    constructor({ txStoreManager, keyManager, owner, hubAddress, url, baseRelayFee, pctRelayFee, gasPriceFactor, web3provider, workerMinBalance = defaultWorkerMinBalance, workerTargetBalance = defaultWorkerTargetBalance, devMode, Debug }) {
        super();
        if (url === undefined) {
    ...
    async createRelayTransaction({ encodedFunction, approvalData, signature, from, to, paymaster, gasPrice, gasLimit, senderNonce, relayMaxNonce, baseRelayFee, pctRelayFee, relayHubAddress }) {
        ...
        try {
            ...
            if (e.message.includes('Returned values aren\'t valid, did it run Out of Gas? You might also see this error if you are not using the correct ABI for the contract you are retrieving data from, requesting data from a block number that does not exist, or querying a node which is not fully synced.')) {
        ...
        let canRelayRet = await this.relayHubContract.methods.canRelay(signedData.message, maxPossibleGas, gasLimits.acceptRelayedCallGasLimit, signature, approvalData).call({ from: this.getAddress(workerIndex) });
        ...
        const maxCharge = parseInt(await this.relayHubContract.methods.calculateCharge(maxPossibleGas, {
            gasPrice,
            pctRelayFee,
            baseRelayFee,
            gasLimit: 0
    ...
    async _worker(blockHeader) {
        try {
            ...
            if (!this.balance || this.balance.lt(toBN(minimumRelayBalance))) {
                throw new StateError(`Server's balance too low ( ${this.balance}, required ${minimumRelayBalance}). Waiting for funding...`);
            }
            const options = {
    ...
    async _registerIfNeeded() {
        ...
        if (!workersAddedEvents.find(e => e.returnValues.newRelayWorkers.map(a => a.toLowerCase()).includes(this.getAddress(1).toLowerCase()))) {
            // register on chain
    ...
    async _resendUnconfirmedTransactionsForSigner(blockHeader, signerIndex) {
        ...
        debug(`resend ${signerIndex}: Removing confirmed txs until nonce ${nonce - 1}. confirmedBlock: ${confirmedBlock}. block number: ${blockHeader.number}`);
